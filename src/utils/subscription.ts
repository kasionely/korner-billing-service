...
export interface SubscriptionRedirectUrls {
  success_url?: string;
  failure_url?: string;
}
...
export const createSubscriptionRequestBody = (
  ...
  redirectUrls?: SubscriptionRedirectUrls
): { data: string; sign: string } => {
  const defaultSuccessUrl = `${config.kornerUrl}/settings/subscription/success?transactionId=${transactionId}`;
  const defaultFailureUrl = `${config.kornerUrl}/failure?transactionId=${transactionId}`;
  ...
  const dataObject = {
    ...
    callback_url: `${config.kornerApiUrl}/api/subscription/callback`,
...
export const createPaymentRecurrentRequestBody = (
  ...
  const dataObject = {
    ...
    description: "Korner recurrent payment",
    ...(process.env.ACTIVE_ENV === "dev" ? { test_mode: 1 } : {}),
  };
